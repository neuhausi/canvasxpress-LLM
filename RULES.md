...
**Grammar-of-graphics shortcut:** when the request is naturally a **ggplot2 + dplyr pipe** (e.g. "scatter of `displ` vs `hwy` colored by `class`, faceted by `drv`"; "group by `cyl`, show mean `mpg`"; "filter to 2008 then a boxplot"), use [GRAMMAR-AUTHORING.md](grammar-authoring.md). CanvasXpress is tidy internally, so the dplyr verbs map 1:1 to config keys (`group_by`→`groupingFactors`, `summarise`→`summaryType`, `mutate`→`transformData`, `filter`→`filterData`, `arrange`→`sortData`, `facet_*`→`segregateSamplesBy`, `pivot_wider`→`pivotBy`) and the ggplot geoms map to `graphType`/`aes`. A true ggplot spec can be authored directly as the `isGGPlot` JSON form. Note: `geom_point`/`geom_line` are `stat_identity` (ungrouped — the grouping column is an aesthetic, NOT `groupingFactors`); only `summary`/`count`/boxplot aggregate.
